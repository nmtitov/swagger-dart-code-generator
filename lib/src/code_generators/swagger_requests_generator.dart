...
class SwaggerRequestsGenerator extends SwaggerGeneratorBase {
            ...
            swaggerPath.requests.forEach((String requestType, SwaggerRequest swaggerRequest) {
                ...
                final returnTypeName = _getReturnTypeName(
                    ...
                    overridenResponses: Map.fromEntries(
                        options.responseOverrideValueMap.where((v) => v.method.isEmpty || v.method == requestType).map((v) => MapEntry(v.url, v)),
                    ),
                );
    ...
    List<String> _getAllMethodModels(
        ...
        for (final successResponse in successResponses) {
            ...
            if (responseRef.isNotEmpty) {
                ...
                results.add(response);
            } else if (successResponse.content?.schema?.allOf.isNotEmpty == true && successResponse.content?.schema?.title.isNotEmpty == true) {
                results.add(response);
            }
        }
    ...
    List<Expression> _getMethodAnnotation({
        ...
        return [
            ...
            if (isUrlencoded) refer(kFactoryConverter.pascalCase).call([], {'request': refer('FormUrlEncodedConverter.requestFactory')}),
            if (isMultipart)
    ...
    String _getParameterTypeName({
        ...
        if (parameter.items?.enumValues.isNotEmpty == true || parameter.schema?.enumValues.isNotEmpty == true || parameter.enumValues.isNotEmpty) {
    ...
    String getChopperClientContent(
        ...
        final baseUrlString = options.withBaseUrl ? "baseUrl:  baseUrl ?? Uri.parse('http://$host$basePath')" : 'baseUrl: baseUrl';

        final converterString = options.withConverter ? 'converter: converter ?? \$JsonSerializableConverter(),' : 'converter: converter ?? chopper.JsonConverter(),';
...
extension SwaggerRootExtension on SwaggerRoot {
    Map<String, SwaggerSchema> get allSchemas => {...definitions, ...components?.schemas ?? {}, ...components?.responses ?? {}, ...components?.requestBodies ?? {}};
}
